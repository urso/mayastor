...
impl TryFrom<&Url> for Lvol {
    ...
    fn try_from(uri: &Url) -> Result<Self, Self::Error> {
        ...
        let lvs = parameters
            ...
            .and_then(|lvs_uri| {
                // Pass through any disk, raid, and strip_size parameters to the LVS URI
                let mut extended_uri = lvs_uri;
                for (key, value) in &parameters {
                    if key == "disk" || key == "raid" || key == "strip_size" {
                        extended_uri.push_str(&format!("&{key}={value}"));
                    }
                }
