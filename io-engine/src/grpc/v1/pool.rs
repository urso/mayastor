...
impl TryFrom<XataRaidConfig> for pool_backend::RaidConfig {
    type Error = LvsError;

    fn try_from(config: XataRaidConfig) -> Result<Self, Self::Error> {
        match config.config {
            Some(xata_raid_config::Config::Raid0(xata_raid0_config)) => {
                Ok(pool_backend::RaidConfig::Raid0(Raid0Config {
                    strip_size_kb: xata_raid0_config.strip_size_kb,
                }))
            }
...
impl From<crate::pool_backend::RaidInfo> for XataRaidInfo {
