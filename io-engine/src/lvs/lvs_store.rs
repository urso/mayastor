...
impl LvsBackendBdevs {
    async fn new(mut args: PoolArgs, create: bool) -> Result<Self, LvsError> {
        ...
        if create {
            info!("Creating or importing {enc} lvs '{name}' from '{disks_str}'...");
        } else {
            info!("Importing {enc} lvs '{name}' from '{disks_str}'...");
        }

        let bdev_ops = Lvs::parse_pool_args(&args)?;
